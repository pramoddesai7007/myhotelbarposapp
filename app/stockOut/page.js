...
const StockOutwardForm = () => {
  ...
  const handleAddItems = async () => {
    try {
      ...
      await axios.post("http://103.159.85.246:6001/api/stockOut/stockOut/addItems", {
        waiterName,
        productName,
        stockQty,
        availableQuantity,
      ...
      await axios.post("http://103.159.85.246:6001/api/item/items/updateQuantity", {
        productName,
        stockQty,
      });
    ...
    try {
      if (name === "waiterName") {
        ...
        const response = await axios.get(
          `http://103.159.85.246:6001/api/waiter/waiter/mobile?name=${value}`
        );
        setMobileNumber(response.data.mobileNumber);
      } else if (name === "productName") {
        setProductName(value);
        const response = await axios.get(
          `http://103.159.85.246:6001/api/item/items/quantity?productName=${value}`
        );
  ...
  const fetchStockOutwardList = async () => {
    try {
      const response = await axios.get(
        "http://103.159.85.246:6001/api/stockOut/stockOut"
      );
