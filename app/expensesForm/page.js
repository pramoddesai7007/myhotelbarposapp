...
const NewExpenseModal = ({ isOpen, onClose }) => {
    ...
    const fetchExpenses = async () => {
      try {
        const response = await axios.get(
          "http://103.159.85.246:6001/api/expense/expenses"
        );
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      await axios.patch(
        `http://103.159.85.246:6001/api/expense/expenses/${selectedExpense._id}`,
        {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post("http://103.159.85.246:6001/api/expense/expenses", {
        expense: newExpense,
      });
...
const ExpenseFormList = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const bankResponse = await axios.get(
        "http://103.159.85.246:6001/api/bankName/bankNames"
      );
      setBankNames(bankResponse.data);
    } catch (error) {
      console.error("Error fetching data:", error);
  ...
  const fetchDataForTable = async () => {
    try {
      const tableResponse = await axios.get(
        "http://103.159.85.246:6001/api/expensesForm"
      );
      setTableData(tableResponse.data);
    } catch (error) {
      console.error("Error fetching table data:", error);
  ...
  const handleDeleteConfirm = async () => {
    try {
      await axios.delete(
        `http://103.159.85.246:6001/api/expensesForm/${deleteExpense._id}`
      );
