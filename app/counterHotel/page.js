...
const CounterHotelForm = () => {
  ...
  const fetchHotels = async () => {
    try {
      const response = await axios.get('http://103.159.85.246:6001/api/counterHotel/get-all');
  ...
  const handleEdit = async (hotelId) => {
    try {
      ...
      const response = await axios.get(`http://103.159.85.246:6001/api/counterHotel/get/${hotelId}`);
...
const handleEditInputChange = (e) => {
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.patch(`http://103.159.85.246:6001/api/counterHotel/edit/${editFormData._id}`, formDataForUpload);
  ...
  const handleDeleteConfirm = async () => {
    try {
      await axios.delete(`http://103.159.85.246:6001/api/counterHotel/delete/${deleteHotelId}`);
      console.log('Hotel deleted successfully.');
      fetchHotels(); // Refresh the list after deletion
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('http://103.159.85.246:6001/api/counterHotel/create', formDataForUpload);
